...
import WebsocketRoute from './ws/WebsocketRoute';

import { wrap_object, to_forward_slashes } from '../shared/operators';

type ServerOptions = uWebSockets.AppOptions & {
    is_ssl: boolean;
    auto_close: boolean;
    fast_abort: boolean;
    trust_proxy: boolean;
...
export default class Server extends Router {
    ...
    #hosts;
    #uws_instance;
    #listen_socket: uWebSockets.us_listen_socket | null;
    #options = {
        is_ssl: false,
        auto_close: true,
        fast_abort: false,
     ...
     * @returns {Object}
     */
    _options = null as unknown as ServerOptions;
     ...
     */
    constructor(options = {} as ServerOptions) {
        ...
        try {
            ...
            if (this.#options.is_ssl) {
                ...
                this.#options.cert_file_name = to_forward_slashes(path.resolve(cert_file_name as string));
                this.#options.key_file_name = to_forward_slashes(path.resolve(key_file_name as string));
            ...
            const _options = Object.keys(options)
                .map((key) => `options.${key}: "${options[key as keyof ServerOptions]}"`)
                .join('\n');
    ...
    async listen(port: number, host = '0.0.0.0'): Promise<uWebSockets.us_listen_socket> {
        ...
        if (this.#options.is_ssl) {
            ...
            try {
                await fs.access(cert_file_name as string);
            } catch (error) {
                throw new Error(
            ...
            try {
                await fs.access(key_file_name as string);
            } catch (error) {
                throw new Error(
     ...
     * @returns {Boolean}
     */
    close(listen_socket?: uWebSockets.us_listen_socket) {
     ...
     * @returns {Boolean}
     */
    publish(topic: string, message: uWebSockets.RecognizedString, is_binary: boolean, compress: boolean) {
     ...
     * @returns {Number}
     */
    num_of_subscribers(topic: string) {
     ...
     */
    _handle_uws_request(route: Route, uws_request: uWebSockets.HttpRequest, uws_response: uWebSockets.HttpResponse, socket: uWebSockets.us_socket_context_t) {
